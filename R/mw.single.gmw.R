...
# Case: MW from the base system, two.sided, X is vector
	    ...
	    { # Our greater and base greater are different interpretations, remeber that!!!
	      testResult <- wilcox.test(X[g==diffTests[testRun,1]],X[g!=diffTests[testRun,1]],alt="two.sided", exact=FALSE)
...
# Case: MW from the base system, greater, X is vector
	    ...
	    { # Our greater and base greater are different interpretations, remember that!!!
	      testResult <- wilcox.test(X[g==diffTests[testRun,1]],X[g!=diffTests[testRun,1]],alt="less", exact=FALSE)
	      PVAL <- testResult$p.value
...
# Case: MW from the base system, smaller, X is vector
	    for(testRun in 1:nrow(diffTests))
	    { # Our greater and base greater are different interpretations, remeber that!!!
	      testResult <- wilcox.test(X[g==diffTests[testRun,1]],X[g!=diffTests[testRun,1]],alt="greater", exact=FALSE)
	      PVAL <- testResult$p.value
...
# Case: permutation, two sided, X is matrix
	   innerLoop <- function(i,testRun){
	     testResult <- wilcox.test(X[g==diffTests[testRun,1],i],X[g!=diffTests[testRun,1],i],alt="two.sided", exact=FALSE)
...
# Case: base, greater, X is matrix
	   innerLoop <- function(i,testRun){
	     testResult <- wilcox.test(X[g==diffTests[testRun,1],i],X[g!=diffTests[testRun,1],i],alt="less", exact=FALSE)
             obsValue <- testResult$statistic
             pValue <- testResult$p.value
	     return(list(pValue=pValue,obsValue=obsValue))
            }
...
# Case: base, smaller, X is matrix
	   innerLoop <- function(i,testRun){
	     testResult <- wilcox.test(X[g==diffTests[testRun,1],i],X[g!=diffTests[testRun,1],i],alt="greater", exact=FALSE)
             obsValue <- testResult$statistic
             pValue <- testResult$p.value
	     return(list(pValue=pValue,obsValue=obsValue))
            }
